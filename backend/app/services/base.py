...
class ServiceBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def create(self, db: Session, *, obj_in: CreateSchemaType, current_user: User) -> ModelType:
        ...
        if hasattr(self.model, "tenant_id"):
            # If not Global Admin, force tenant_id
            if current_user.role != UserRole.GLOBAL_ADMIN:
                obj_in_data["tenant_id"] = current_user.tenant_id
            # If Global Admin, tenant_id should be in obj_in or handled by caller
            # If missing for Global Admin, it might fail DB constraint if nullable=False
